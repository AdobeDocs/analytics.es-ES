---
title: Cómo establecer las preferencias de usuario y empresa en Analysis Workspace
...
Vea este vídeo para ver una breve descripción general de las preferencias:

>[!VIDEO](https://video.tv.adobe.com/v/332600/?quality=12)

## Actualizar preferencias

1. En Adobe Analytics, vaya a la [!UICONTROL **Proyecto**] página de aterrizaje y, a continuación, seleccione [!UICONTROL **Preferencias**].

   ![Preferencias del usuario](assets/user-preferences.png)

1. Para obtener información sobre las preferencias disponibles en cada ficha, continúe con cualquiera de las secciones siguientes de este artículo:

   * [Preferencias generales](#general-preferences)

   * [Preferencias de proyecto](#project-preferences)

   * [Preferencias de tabla improvisada](#freeform-table-preferences)

   * [Preferencias de visualizaciones](#visualizations-preferences)

## Preferencias generales

Puede personalizar las preferencias generales de todos los proyectos nuevos que cree en Analysis Workspace. Para obtener información sobre cómo acceder a estas preferencias, consulte [Actualizar preferencias](#update-preferences).
...
| Componentes mostrados en los grupos del carril izquierdo | Elija cuántos de cada componente se van a mostrar en el menú Componentes del carril izquierdo. <p>Si elige 0, ya no se puede acceder al componente desde el carril izquierdo de los espacios de trabajo.</p><p>De forma predeterminada, se muestran 5 componentes para cada una de las siguientes opciones:</p> <ul><li>Dimensiones</li><li>Métricas</li><li>Filtros</li><li>Intervalos de fechas</li></ul> <p>Para obtener más información sobre los componentes en Analysis Workspace, consulte [Información general sobre componentes](/help/analyze/analysis-workspace/components/analysis-workspace-components.md).</p> |

## Preferencias de proyecto

Puede personalizar las preferencias del proyecto para todos los proyectos nuevos que cree en Analysis Workspace. Para obtener información sobre cómo acceder a estas preferencias, consulte [Actualizar preferencias](#update-preferences).

Algunas de estas mismas preferencias también se pueden personalizar para proyectos individuales, como se describe en [Información general del proyecto](/help/analyze/analysis-workspace/build-workspace-project/freeform-overview.md).

Haga clic en los títulos de preferencias vinculados para obtener más información y contexto sobre cada preferencia.
...
|  | Mostrar anotaciones | Elija si las anotaciones están visibles en los proyectos. Para obtener más información sobre anotaciones, consulte [Resumen de anotaciones](/help/analyze/analysis-workspace/components/annotations/overview.md). |

## Preferencias de tabla improvisada

Puede personalizar las preferencias de tabla improvisada para todos los proyectos nuevos que cree en Analysis Workspace. Para obtener información sobre cómo acceder a estas preferencias, consulte [Actualizar preferencias](#update-preferences).
...
|  | Alinear fechas | Seleccione esta opción para alinear las fechas de cada columna con todas a partir de la misma fila. |
...
|  | Mostrar minigráfico | Muestre u oculte gráficos de líneas en la parte inferior del gráfico. Cuando está oculta, la leyenda cambia para no hacer referencia visualmente a las líneas. |
...
|  | Leyenda visible | Le permite ocultar el texto de leyenda detallado para todas las visualizaciones. |
...
|  | Permitir que las anomalías escalen el eje Y | Si tiene varias métricas en un gráfico, debe pasar el ratón sobre cada anomalía para ver la banda de confianza de esa métrica. Para que la visualización sea más legible, el intervalo de confianza Detección de anomalías no escala automáticamente el eje Y. Esta opción permite que el intervalo de confianza escale la visualización. <p>Para obtener más información, consulte [Ver anomalías en Analysis Workspace](/help/analyze/analysis-workspace/virtual-analyst/c-anomaly-detection/view-anomalies.md).</p> |
...
|  | Leyenda visible | Permite ocultar el texto detallado del pie de ilustración para la visualización Línea. |
|  | Límite máximo de elementos | Reduce el número de elementos en el eje X de la visualización Línea. Esto puede resultar útil si tiene un conjunto de datos grande. |
|  | Mostrar doble eje (cuando corresponda) | Solo es aplicable si cuenta con dos métricas: puede tener un eje Y a la izquierda (para una métrica) y a la derecha (para otra métrica). Esto resulta útil cuando las métricas trazadas son de magnitudes muy diferentes. |
|  | Normalización (cuando corresponda) | Fuerza métricas para igualar proporciones. Esto resulta útil cuando las métricas trazadas son de magnitudes muy diferentes. |
|  | Mostrar eje X | Muestra el eje x en el gráfico de líneas. |
|  | Mostrar eje Y | Muestra el eje Y en el gráfico de líneas. |
|  | Eje Y delimitador | Si todos los valores marcados en el gráfico están considerablemente por encima de cero, el gráfico mostrará el valor base del eje Y distinto a cero. Si marca esta casilla, el eje Y se forzará a ser cero (y se redibujará el gráfico). |
...
|  | Mostrar línea de tendencia | Muestre una regresión o una línea de tendencia promedio móvil a la serie de líneas. Las líneas de tendencia ayudan a mostrar un patrón más claro en los datos. |
...
|  | Mostrar sólo porcentaje | Quita el valor numérico y solo muestra el porcentaje. |
...
|  | Mostrar fila de porcentaje promedio | Inserta una nueva fila en la parte superior de la tabla y agrega el promedio para los valores dentro de cada columna. |
...
|  | Mostrar puntos en las líneas | Muestre las campanas en líneas en gráficos combinados. |
...
|  | Mostrar minigráficos | ocultar u ocultar gráficos de líneas en la parte inferior del gráfico. Cuando está oculta, la leyenda cambia para no hacer referencia visualmente a las líneas. |
|  | Mostrar máx. y mín. en los reflectores | Mostrar valores mínimos y máximos en gráficos de líneas principales y de comparación. |
|  | Mostrar comparación | Mostrar datos de comparación. Cuando están ocultos, el gráfico de líneas de comparación y los objetos de cambio de resumen no se ven. |
|  | Opciones de valor numérico | En el [!UICONTROL **Resumen de métricas clave**] sección <ul><li>Mostrar cambio de porcentaje</li><li>Mostrar diferencia en bruto</li>Diferencia en bruto entre el valor total de la métrica en el intervalo de fechas principal y el intervalo de fechas secundario</ul> |
...
|  | Contenedor | En el [!UICONTROL **Flujo**] sección <ul><li>Visita</li><li>Visitante.</li></ul> |
...
|  | Mostrar información sobre herramientas | Determina si se muestran las informaciones de objeto que contienen datos de nodo al pasar el ratón por encima de nodos individuales dentro de una visualización de flujo. |
...
| **Gráficos apilados** |  |  |
|  | Apilado al 100 % | Esta configuración en las visualizaciones de áreas apiladas, barras apiladas o barras horizontales apiladas convierte el gráfico en una visualización “apilada al 100 %”.  <p>Para obtener más información, consulte [Barra y barra apilada](/help/analyze/analysis-workspace/visualizations/bar.md).</p> |
| **[Histograma](/help/analyze/analysis-workspace/visualizations/histogram.md)** |  |  |
|  | Número de bloques | Elija el número de intervalos de datos (contenedores) en la visualización. El número máximo de contenedores es 50. <p>Para obtener más información, consulte [Histograma](/help/analyze/analysis-workspace/visualizations/histogram.md).</p> |
|  | Método de recuento | Elija entre las siguientes opciones: <ul><li>Visita individual</li><li>Visita</li><li>Visitante.</li></ul> <p>Por ejemplo, cuando se usa junto con las vistas de página, puede elegir las vistas de página por visitante, las vistas de página por visita o las vistas de página por visita. Para visitas, se utiliza “Ocurrencias” como métrica del eje y en una tabla de forma libre.</p> |
...
|  | Leyenda visible | Permite ocultar el texto detallado del pie de ilustración para la visualización Cambio de resumen . |
| **[Número de resumen](/help/analyze/analysis-workspace/visualizations/summary-number-change.md)** |  |  |
|  | Porcentajes | Muestra valores en porcentajes para las visualizaciones Número de resumen . |
|  | Leyenda visible | Permite ocultar el texto detallado del pie de ilustración para la visualización Número de resumen . |
|  | Valor de resumen por | Elija entre Max, Min, Mean, Median y Sum. |
|  | Valor abreviado | En el [!UICONTROL **Número de resumen**] sección |
| **[Gráfico de rectángulos](/help/analyze/analysis-workspace/visualizations/treemap.md)** |  |  |
|  | Porcentajes | Muestra valores en porcentajes para las visualizaciones de gráficos de rectángulos. |
|  | Límite máximo de elementos | Reduce el número de elementos en el eje X de la visualización del diagrama de árbol. Esto puede resultar útil si tiene un conjunto de datos grande. |
| **[Venn](/help/analyze/analysis-workspace/visualizations/venn.md)** |  |  |
|  | Leyenda visible | Le permite ocultar el texto de leyenda detallado para la visualización Venn. |
...
|  | Leyenda visible | Le permite ocultar el texto de leyenda detallado para la visualización de Dispersión. |
|  | Límite máximo de elementos | Reduce el número de elementos en el eje X de la visualización de puntos. Esto puede resultar útil si tiene un conjunto de datos grande. |
|  | Anclar eje Y a cero | Si todos los valores marcados en el gráfico están considerablemente por encima de cero, el gráfico mostrará el valor base del eje Y distinto a cero. Si marca esta casilla, el eje Y se forzará a ser cero (y se redibujará el gráfico). |

## Preferencias de empresa

>[!AVAILABILITY]
>
>La funcionalidad Vínculos de acceso público que se describe en esta sección se encuentra en la fase Prueba limitada de la versión. Esta nota se eliminará cuando la funcionalidad esté disponible en general. Para obtener información sobre el proceso de versión de Analytics, consulte [Versiones de funcionalidades de Adobe Analytics](/help/release-notes/releases.md).
...
| --- | --- | --- |
| **Pestaña de informes** |  |  |
|  | Ficha Ocultar informes | Oculta la ficha Informes para todos los usuarios de la organización. |
| **Vínculos de acceso público** <!-- Double check the names of all these settings for what they are actually called --> |  |  |
|  | Deshabilitar todos los vínculos de acceso público | <p>Impide que los usuarios de su organización compartan proyectos de Analysis Workspace con personas que no tienen una cuenta de Analysis Workspace. Los usuarios solo pueden compartir proyectos con otros usuarios de Analysis Workspace de su organización.</p> <p>Cuando los vínculos públicos están desactivados:</p> <ul><li><p>Los usuarios no pueden crear vínculos de acceso público</p><p>La opción &quot;Compartir vínculo público&quot; se ha eliminado del menú Compartir. Esto significa que los usuarios ya no pueden compartir proyectos con personas que no tengan una cuenta de Analysis Workspace en su organización, tal como se describe en [Compartir un vínculo público con cualquier persona (no se requiere inicio de sesión)](/help/analyze/analysis-workspace/curate-share/share-projects.md#share-public-link) en [Compartir proyectos](/help/analyze/analysis-workspace/curate-share/share-projects.md).</p></li> <li><p>Cualquier vínculo público existente está deshabilitado</p></li><p>Si los vínculos públicos están desactivados y luego se vuelven a habilitar, todos los vínculos desactivados anteriormente no se reactivan automáticamente. En este caso, los usuarios deben reactivarlos manualmente para cada proyecto desde el cuadro de diálogo Compartir proyecto .</p> |
|  | Requerir contraseña | <p>Aplica a los usuarios de su organización que necesitan protección mediante contraseña cuando crean vínculos públicos.</p> <p>Una vez habilitada esta opción, cada vez que un usuario cree un vínculo público a un proyecto de Analysis Workspace, la opción &quot;Requerir contraseña&quot; se habilitará en el cuadro de diálogo de uso compartido y el usuario no podrá desactivarla. (Para obtener información sobre cómo los usuarios pueden compartir proyectos con usuarios fuera de la organización, consulte [Compartir un vínculo público con cualquier persona (no se requiere inicio de sesión)](/help/analyze/analysis-workspace/curate-share/share-projects.md#share-public-link) en [Compartir proyectos](/help/analyze/analysis-workspace/curate-share/share-projects.md).)</p><p>Tenga en cuenta lo siguiente al habilitar esta opción:</p> <ul><li><p>Cuando activa esta opción, se desactivan todos los vínculos públicos activos actualmente que no tengan la protección de contraseña habilitada.</p></li> <li><p>Si esta opción está habilitada y después deshabilitada, todos los vínculos previamente desactivados no se reactivan automáticamente. En este caso, los usuarios deben reactivarlos manualmente desde el cuadro de diálogo Compartir proyecto .</p></li></ul> |
|  | Requerir autenticación de inicio de sesión único | <p>Requiere autenticación de inicio de sesión único (SSO) para todos los vínculos públicos. Cuando esta opción está habilitada, solo los destinatarios que tengan un Federated ID y puedan iniciar sesión en Experience Cloud pueden acceder a los vínculos públicos compartidos.</p> <p>Una vez habilitada esta opción, cada vez que un usuario cree un vínculo a un proyecto de Analysis Workspace, la opción &quot;Requerir autenticación de inicio de sesión único (SSO)&quot; estará activada en el cuadro de diálogo de uso compartido y el usuario no podrá desactivarla. (Para obtener información sobre cómo los usuarios pueden compartir proyectos con usuarios fuera de la organización, consulte [Compartir un vínculo público con cualquier persona (no se requiere inicio de sesión)](/help/analyze/analysis-workspace/curate-share/share-projects.md#share-public-link) en [Compartir proyectos](/help/analyze/analysis-workspace/curate-share/share-projects.md).)</p> <p>Tenga en cuenta lo siguiente al habilitar esta opción:</p><ul><li><p>Cuando activa esta opción, todos los vínculos públicos actualmente activos que no tienen SSO habilitado se desactivan.</p></li> <li><p>Si esta opción está habilitada y después deshabilitada, todos los vínculos previamente desactivados no se reactivan automáticamente. En este caso, los usuarios deben reactivarlos manualmente desde el cuadro de diálogo Compartir proyecto .</p></li> <li><p>Esta opción solo está disponible si SSO está implementado en su organización. Para obtener información sobre cómo los administradores del sistema pueden habilitar SSO para su organización, consulte [Configuración de identidad e inicio de sesión único](https://helpx.adobe.com/es/enterprise/using/set-up-identity.html){target=_blank}.</p><p>Si SSO está configurado para su organización, compruebe si algún tipo de creación de cuenta automática está implementado en la consola. Normalmente, un administrador del sistema lo configuraría, tal como se describe en [Habilitar la creación automática de cuentas](https://helpx.adobe.com/enterprise/using/automatic-account-creation.html){target=_blank}.</p></li><li><p>Si su organización está en un sector que requiere cumplimiento HIPAA, esta opción se habilita automáticamente y no se puede deshabilitar.</p></li></ul> |
|  | Mostrar la opción &quot;Requerir autenticación de inicio de sesión único&quot; a los usuarios | <p>Si la opción &quot;Requerir autenticación de inicio de sesión único&quot; está desactivada, puede seleccionar esta opción para permitir a los usuarios de su organización elegir si desean requerir autenticación de inicio de sesión único para los proyectos que comparten. En este caso, cada vez que un usuario crea un vínculo a un proyecto de Analysis Workspace, la opción &quot;Requerir autenticación de inicio de sesión único (SSO)&quot; está habilitada en el cuadro de diálogo de uso compartido. Los usuarios pueden desactivarlo si lo desean.<!--or is this disabled by default?--> (Para obtener información sobre cómo los usuarios pueden compartir proyectos con usuarios fuera de la organización, consulte [Compartir un vínculo público con cualquier persona (no se requiere inicio de sesión)](/help/analyze/analysis-workspace/curate-share/share-projects.md#share-public-link) en [Compartir proyectos](/help/analyze/analysis-workspace/curate-share/share-projects.md).)</p><p>Si la opción &quot;Requerir autenticación de inicio de sesión único&quot; está activada, esta opción para mostrarla a los usuarios está seleccionada y no se puede anular la selección.</p><p>Tenga en cuenta lo siguiente al seleccionar esta opción:</p><ul><li><p>Esta opción solo está disponible si SSO está implementado en su organización. Para obtener información sobre cómo los administradores del sistema pueden habilitar SSO para su organización, consulte [Configuración de identidad e inicio de sesión único](https://helpx.adobe.com/es/enterprise/using/set-up-identity.html){target=_blank}.</p><p>Si SSO está configurado para su organización, compruebe si algún tipo de creación de cuenta automática está implementado en la consola. Normalmente, un administrador del sistema lo configuraría, tal como se describe en [Habilitar la creación automática de cuentas](https://helpx.adobe.com/enterprise/using/automatic-account-creation.html){target=_blank}.</p></li><li><p>Si selecciona esta opción y un usuario decide requerir la autenticación de inicio de sesión único, solo los destinatarios que tengan un Federated ID y puedan iniciar sesión en Experience Cloud podrán acceder a los vínculos públicos compartidos.</p></li></ul> |

{style=&quot;table-layout:auto&quot;}

## Restaurar preferencias predeterminadas

Puede restaurar todas sus preferencias de usuario a los valores predeterminados del sistema. Esto no afecta a las preferencias del administrador en la ficha Empresa.
