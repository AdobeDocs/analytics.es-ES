---
title: Vínculos de exclusión
description: Obtenga información sobre cómo crear e implementar vínculos de exclusión para los visitantes del sitio.
...
# Implementación de vínculos de exclusión

> [!IMPORTANT] Adobe recomienda utilizar el servicio de inclusión, especialmente para las organizaciones que se ocupan de las regulaciones del RGPD. See [Opt-in service overview](https://docs.adobe.com/content/help/en/id-service/using/implementation/opt-in-service/optin-overview.html) in the Experience Cloud Identity Service user guide.

Algunos visitantes del sitio Web prefieren no incluir su información de navegación en el conjunto de datos. Adobe ofrece la posibilidad de proporcionar a los visitantes de su sitio web un medio para excluir la recopilación de su información. Se admiten todos los tipos de implementación; su organización es responsable de su propia política de privacidad y de cumplir con los términos firmados.
...
La página de exclusión de su organización depende del valor de la [`trackingServer`](../vars/config-vars/trackingserver.md) variable en la implementación.
...
* En una implementación de JavaScript:
   ...
   2. Observe el valor de la `trackingServer` variable.
...
* Using the [Adobe Experience Cloud Debugger](https://docs.adobe.com/content/help/en/debugger/using/experience-cloud-debugger.html):
   1. Navegue a su sitio mediante el navegador Chrome.
   2. Abra Experience Cloud Debugger y vaya a la ficha Red.
   3. Tenga en cuenta el valor [!UICONTROL Solicitar URL - Nombre de host] .

Una vez encontrado el dominio de la implementación, anexe la ruta `trackingServer` `/optout.html` al final. Por ejemplo:
...
Cambiar automáticamente el idioma de la página de exclusión incluyendo el parámetro de cadena de `locale` consulta. Asigne este parámetro de cadena de consulta uno de los siguientes valores:
...
Agrega un botón &#39;Cerrar ventana&#39; a la página, lo que permite que la página de exclusión se convierta en una ventana emergente. Utilice el parámetro de cadena de `popup` consulta y asígnele un valor de `1`.

Por ejemplo, `https://example.sc.omtrdc.net/optout.html?popup=1` carga la página de exclusión con el botón &#39;Cerrar ventana&#39;.

> [!NOTE] Históricamente, este parámetro de cadena de consulta forzó una ventana emergente. Sin embargo, la mayoría de los exploradores modernos le dan control sobre las ventanas emergentes al usuario final.
