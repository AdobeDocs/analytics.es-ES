...
* **`visitorMigrationKey`**: llevaba una clave utilizada para migrar visitantes de cookies de terceros a cookies de origen. Se ha retirado porque las bibliotecas modernas establecen una cookie de reserva (`fid`) de origen y dependen del servicio de ID de visitante para su identidad.
...
* **`visitorNameSpace`**: ayudó a determinar el dominio de cookies de terceros. Se reemplaza por el uso de la variable [`trackingServerSecure`](trackingserversecure.md) en implementaciones que no usan el servicio de ID de visitante.
