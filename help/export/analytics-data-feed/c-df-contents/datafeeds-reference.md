...
Utilice esta página para conocer los datos contenidos en cada columna. La mayoría de las implementaciones no utiliza todas las columnas, por lo que se puede hacer referencia a esta página al determinar qué columnas incluir en una exportación de fuente de datos.

> [!IMPORTANT] Para cualquier columna dada (por ejemplo, una que se define como 255 caracteres), una fuente de datos puede enviar caracteres adicionales debido a la adición de caracteres que escapan los valores de una cadena. Tenga en cuenta este tema si la implementación envía regularmente valores que excedan los límites de caracteres.

## Columnas, descripciones y tipos de datos

> [!NOTE] La mayoría de las columnas contiene una columna similar con un prefijo de `post_`. Las columnas de publicación muestran valores después de la lógica del lado del servidor, las reglas de procesamiento y las reglas de VISTA. Adobe recomienda usar columnas de publicación en la mayoría de los casos.
...
| browser_height | Altura de la ventana del explorador en píxeles. | smallint unsigned |
| browser_width | Anchura de la ventana del explorador en píxeles. | smallint unsigned |
...
| click_action_type | Ya no se utiliza. Tipo de vínculo de la herramienta heredada Clickmap.<br>0: HREF URL<br>1: ID personalizado<br>2: Evento onclick de JavaScript<br>3: Elemento de formulario | tinyint sin firmar |
...
| click_context_type | Ya no se utiliza. Indica si click_context tenía un nombre de página o si estaba vinculado a la URL de la página de forma predeterminada.<br>0: URL de la página<br>1: Nombre de página | tinyint sin firmar |
...
| clickmaplinkbyregion | Vínculo Mapa de actividades por región | varchar(255) |
| clickmappage | Página Activity Map | varchar(255) |
| clickmapregion | Región Mapa de actividades | varchar(255) |
...
| color | ID de profundidad de color basada en el valor de la columna c_color. Se remite a la tabla de búsqueda de color_depth.tsv. | smallint unsigned |
...
| cookies | Variable utilizada en la dimensión Compatibilidad con cookies.<br>Y: Enabledn<br>: Disabledu<br>: Desconocido | char(1) |
...
| exclude_hit | Un indicador que señala que la visita no está incluida en la creación de informes. La columna visit_ num no se incrementa en las visitas excluidas.<br>1: No se utiliza. Parte de una función eliminada.<br>2: No se utiliza. Parte de una función eliminada.<br>3: Ya no se utiliza. Exclusión del agente de usuario<br>4: Exclusión basada en la dirección IP<br>5: Falta información de visita crítica, como page_ url, pagename, page_ event o event_ list<br>6: JavaScript no procesó correctamente la visita de<br>7: Exclusión específica de la cuenta, como en las reglas de VISTA<br>8: No se utiliza. Excepción alternativa de cuenta alternativa.<br>9: No se utiliza. Parte de una función eliminada.<br>10: Código de moneda no válido<br>11: Hit missing a timestamp on a timestamp-only report suite, or a hit contained a timestamp on a non-timestamp report suite<br>12: No se utiliza. Parte de una función eliminada.<br>13: No se utiliza. Parte de una función eliminada.<br>14: Visita de Target que no coincide con una visita de Analytics<br>15: No se utiliza actualmente.<br>16: Visita de Marketing Cloud que no coincide con una visita de Analytics | tinyint sin firmar |
...
| geo_zip | El código postal de la visita proveniente de, basado en IP. Adobe se asocia con Digital Envoy para hacer coincidir las direcciones IP con los códigos postales. | varchar(16) |
...
| hit_source | Indica la fuente de la que provino la visita. <br>1: Solicitud de imagen estándar sin marca de tiempo <br>2: Solicitud de imagen estándar con marca de tiempo <br>3: Carga de fuente de datos Live con marcas de hora <br>4: No se utiliza <br>5: Carga de fuentes de datos genérica <br>6: Carga de fuentes de datos de procesamiento completo <br>7: Carga de fuentes de datos transactionid <br>8: Ya no se utiliza; Versiones anteriores de fuentes de datos de Adobe Advertising Cloud <br>9: Ya no se utiliza; Métricas de resumen de Adobe Social | tinyint sin firmar |
| hit_time_gmt | La marca de tiempo de los servidores de recopilación de datos de Adobe recibidos recibió la visita, según la hora Unix. | int |
...
| java_enabled | Indicador que señala si Java está habilitado. <br>Y: Habilitado <br>N: U deshabilitado <br>: Desconocido | char(1) |
...
| language | ID numérica de idioma. Utiliza la tabla de búsqueda de languages.tsv. | smallint unsigned |
...
| last_purchase_num | Variable utilizada en la dimensión Lealtad del cliente. Indica la cantidad de compras anteriores que realizó el visitante. <br>0: No hay compras anteriores (cliente) <br>1: 1 compra anterior (cliente nuevo) <br>2: 2 compras anteriores (cliente que retorna) <br>3: 3 o más compras anteriores (cliente fiel) | int sin firmar |
...
| mobileappid | ID de la aplicación móvil. Almacena el nombre y la versión de la aplicación en el siguiente formato:[Paquete] [de appname] | varchar(255) |
...
| mobileappstoreobjectid | Se utiliza en el conector de datos Appfigures. ID del objeto App Store | varchar(255) |
| mobilebeaconmajor | Señalización de Mobile Services | varchar(100) |
| mobilebeaconminor | Señalización de Mobile Services menor | varchar(100) |
| mobilebeaconproximity | Proximidad de señalización de Mobile Services | varchar(255) |
| mobilebeaconuuid | UUID de Mobile Services UUID | varchar(100) |
...
| mobiledayssincelastupgrade | Se recopilan de la variable de datos de contexto a. dayssincelastupgrade. Cantidad de días que pasaron desde la sesión anterior. | varchar(255) |
| mobiledayssincelastuse | Cantidad de días desde que la aplicación se ejecutó por última vez. | varchar(255) |
| mobiledeeplinkid | Se recopiló de la variable de datos de contexto a.<span>deeplink</span>. id. Se utiliza en los informes de adquisición como identificador para el vínculo de adquisición móvil. | varchar(255) |
...
| mobilelaunchessincelastupgrade | Se recopila de la variable de datos de contexto a. launchessinceupgrade. Informa el número de inicios desde la última actualización. | varchar(255) |
...
| mobilemessagebuttonname | Se recopiló de la variable de datos de contexto a.<span>message</span>. button. id. Se utiliza para la mensajería en la aplicación para identificar el botón que cerró el mensaje. | varchar(100) |
| mobilemessageid | ID de mensaje en la aplicación | varchar(255) |
| mobilemessageonline | Mensaje en la aplicación en línea | varchar(255) |
| mobilemessagepushoptin | Se recopiló de la variable de datos de contexto a. push. optin. Se establece en "true" cuando el usuario opta por activar la mensajería; De lo contrario, el valor es "false". | varchar(255) |
| mobilemessagepushpayloadid | Se recopila de la variable de datos de contexto a. push. payloadid. Se utiliza en la mensajería push como identificador de carga útil. | varchar(255) |
| mobileosenvironment | Se recopila en la variable de datos de contexto a. osenvironment. Entorno del sistema operativo Estados, como Android o iOS. | varchar(255) |
| mobileosversion | Versión del sistema operativo de Mobile Services | varchar(255) |
| mobileplaceaccuracy | Se recopiló de la variable de datos de contexto a. loc. acc. Indica la precisión del GPS en metros en el momento de la colección. | varchar(255) |
| mobileplacecategory | Se recopiló en la variable de datos de contexto a. loc. category. Describe la categoría de un lugar específico. | varchar(255) |
| mobileplaceid | Se recopiló de la variable de datos de contexto a.<span>loc</span>. id. Identificador para un punto de interés determinado. | varchar(255) |
...
| mobilerelaunchcampaignsource | Origen de inicio de Mobile Services | varchar(255) |
| mobilerelaunchcampaignterm | Término de inicio de Mobile Services | varchar(255) |
| mobilerelaunchcampaigntrackingcode | Se recopiló de la variable de datos de contexto a. launch. campaign. trackingcode. Se utiliza en la adquisición como código de seguimiento para la campaña de inicio. | varchar(255) |
...
| mvvar 1 - mvvar 3 | Valores de variable de lista. Contiene una lista delimitada de valores personalizados en función de la implementación. | text |
...
| page_event | El tipo de visita que se envía en la solicitud de imagen (visita estándar, vínculo de descarga, vínculo personalizado, vínculo de salida). | tinyint sin firmar |
...
| t_time_info | Hora local del visitante. El formato es el siguiente: M/D/AAAA HH: MM: Desfase de Zona horaria (en minutos) del mes SS (0-11, 0 = enero). | varchar(100) |
...
| truncated_hit | Un indicador que señala que se ha truncado la solicitud de imagen. Indica que se ha recibido una visita parcial. <br>Y: La visita se truncó; visita parcial recibida <br>N: La visita no se truncó; visita completa recibida | char(1) |
...
| user_hash | No es de uso. Hash de la ID del grupo de informes. Se usa username en su lugar. | int sin firmar |
...
| videoaudioalbum | Álbum de audio de vídeo | varchar(255) |
| videoaudioartist | Artista de audio de vídeo | varchar(255) |
...
| videocampaign | Campaña de vídeo | varchar(255) |
...
| videodaypart | Partición de día de vídeo | varchar(255) |
...
| videoqoebuffertimeevar | Tiempo de búfer de calidad de vídeo | varchar(255) |
...
| videoqoeplayersdkerrors | Errores de SDK de calidad de vídeo | text |
...
| videoshow | Mostrar vídeo | varchar(255) |
| videoshowtype | Tipo de visualización de vídeo | varchar(255) |
...
| visid_type | ID numérica que representa qué método se ha utilizado para identificar al visitante. <br>0: Visitorid personalizado <br>1: IP y agente de usuario fallback <br>2: Encabezado del suscriptor móvil HTTP <br>3: Valor de cookie heredada (s_ vi) <br>4: Valor de cookie de reserva (s_ fid) <br>5: Servicio de identidad | tinyint sin firmar |
| visit_keywords | Variable utilizada en la dimensión Palabra clave de búsqueda. Esta columna utiliza un límite de caracteres no estándar para dar cabida a la lógica utilizada por Adobe. | varchar(244) |
...
* mobileappstoreobjectid<span>.</span>fabricante del dispositivo
...
* mobileappstoreobjectid<span>.</span>classification-category-type
* mobileappstoreobjectid<span>.</span>region-name
* mobileappstoreobjectid<span>.</span>revisar comentario
* mobileappstoreobjectid<span>.</span>review-title
* mobileappstoreoneoffrevenue
* mobileappstoreoneoffroycounts
* mobileappstorecompras
* mobileappstorerank
