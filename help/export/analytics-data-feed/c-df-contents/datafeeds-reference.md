...
| **`aemassetid`** | Variable de varios valores que corresponde a los ID de recurso (GUID) de un conjunto de recursos de Adobe Experience Manager. Aumenta los eventos de impresión. | texto |
...
| **`channel`** | La dimensión [Secciones del sitio](/help/components/dimensions/site-section.md). | varchar(100) |
...
| **`connection_type`** | Un ID numérico que representa el tipo de conexión. La dimensión [Tipo de conexión](/help/components/dimensions/connection-type.md). Se remite a la tabla de búsqueda `connection_type.tsv`. | tinyint sin firmar |
...
| **`currency`** | El código de moneda que se ha utilizado durante la transacción. Configurado mediante [`currencyCode`](/help/implement/vars/config-vars/currencycode.md). | char(8) |
...
| **`cust_visid`** | El ID de visitante personalizado, si se establece mediante [`visitorID`](/help/implement/vars/config-vars/visitorid.md). | varchar(255) |
| **`customer_perspective`** | Determina si la visita fue una visita móvil en segundo plano. Consulte [Sesiones según el contexto](/help/components/vrs/vrs-mobile-visit-processing.md) para obtener más información. | tinyint sin firmar |
...
| **`mobiledevice`** | Nombre del dispositivo móvil. En iOS, se almacena como una cadena de 2 dígitos separados por comas. El primer número representa la generación del dispositivo y el segundo representa miembros diferentes de la familia del dispositivo. | varchar(255) |
...
| **`mvvar1`** - `mvvar3` | Valores de [lista de variable](/help/implement/vars/page-vars/list.md). Contiene una lista delimitada de valores personalizados en función de la implementación. Las columnas `post_mvvar1` - `post_mvvar3` reemplazan el delimitador original por `--**--`. | texto |
...
| **`page_event_var1`** | Solo se utiliza en solicitudes de imagen de seguimiento de vínculos. URL del vínculo de descarga, de salida o personalizado en el que se hizo clic. | texto |
| **`page_event_var2`** | Solo se utiliza en solicitudes de imagen de seguimiento de vínculos. Nombre personalizado (si se especifica) del vínculo. Establece el [vínculo personalizado](/help/components/dimensions/custom-link.md), el [vínculo de descarga](/help/components/dimensions/download-link.md) o el [vínculo de salida](/help/components/dimensions/exit-link.md), según el valor de `page_event`. | varchar(100) |
...
| **`page_url`** | La URL de la visita. Tenga en cuenta que `post_page_url` se elimina para las solicitudes de imagen de seguimiento de vínculos ([`tl()`](/help/implement/vars/functions/tl-method.md)) y utiliza un tipo de datos de varchar (255). | texto |
| **`pagename`** | Dimensión [Página](/help/components/dimensions/page.md). Si la variable [`pagename`](/help/implement/vars/page-vars/pagename.md) está vacía, Analytics utiliza `page_url` en su lugar. | varchar(100) |
...
| Columnas **`post_`** | Contiene el valor que se utiliza finalmente en los informes. Cada columna de publicación se rellena tras la lógica del lado del servidor, las reglas de procesamiento y las reglas VISTA. Adobe recomienda usar columnas de publicación en la mayoría de los casos. | Consulte la columna de no publicación correspondiente |
...
| **`ref_type`** | ID numérico que representa el tipo de referente de la visita. Se utiliza en la dimensión [Tipo de referente](/help/components/dimensions/referrer-type.md). <p>1: Dentro del sitio</p><p>2: Otros sitios web</p> <p>3: Motores de búsqueda</p> <p>4: Disco duro</p> <p>5: USENET</p> <p>6: Escrito/Añadido como marcador (sin referente)</p> <p>7: Correo electrónico</p> <p>8: Sin JavaScript</p> <p>9: Redes sociales</p><p>10: Herramientas de IA conversacional</p> | tinyint sin firmar |
| **`referrer`** | Dimensión [Referente](/help/components/dimensions/referrer.md). Tenga en cuenta que mientras `referrer` utiliza un tipo de datos de varchar(255), `post_referrer` emplea un tipo de datos de varchar(244). | varchar(255) |
...
| **`search_engine`** | ID numérico que representa el motor de búsqueda que redirigió al visitante a su sitio. Se utiliza en dimensiones del [motor de búsqueda](/help/components/dimensions/search-engine.md). Se remite a la tabla de búsqueda `search_engines.tsv`. | smallint sin firmar |
...
| **`truncated_hit`** | Un indicador que indica que se ha truncado la solicitud de imagen. Indica que se ha recibido una visita parcial. <br>S: Se truncó la visita; visita parcial recibida <br>N: La visita no se truncó; visita completa recibida | char(1) |
...
| **`video`** | Dimensión de servicios de medios de streaming [Contenido](/help/components/dimensions/sm-core.md). | varchar(255) |
| **`videoad`** | Dimensión de servicios de medios de streaming [Publicidad](/help/components/dimensions/sm-ads.md). | varchar(255) |
| **`videoadinpod`** | Dimensión de servicios de medios de streaming [Posición del anuncio en la secuencia](/help/components/dimensions/sm-ads.md). | varchar(255) |
| **`videoadlength`** | Dimensión de servicios de medios de transmisión [Longitud del anuncio (variable)](/help/components/dimensions/sm-ads.md). | entero |
| **`videoadload`** | Dimensión de servicios de medios de streaming [Cargas del anuncio](/help/components/dimensions/sm-ads.md). | varchar(255) |
| **`videoadname`** | Dimensión de servicios de medios de streaming [Nombre del anuncio (variable)](/help/components/dimensions/sm-ads.md). | varchar(255) |
| **`videoadplayername`** | Dimensión de servicios de medios de streaming [Nombre del reproductor del anuncio](/help/components/dimensions/sm-ads.md). | varchar(255) |
| **`videoadpod`** | Dimensión de servicios de medios de streaming [Pod del anuncio](/help/components/dimensions/sm-ads.md). | varchar(255) |
| **`videoadvertiser`** | Dimensión de servicios de medios de streaming [Anunciante](/help/components/dimensions/sm-ads.md). | varchar(255) |
| **`videoaudioalbum`** | Dimensión de servicios de medios de streaming [Álbum](/help/components/dimensions/sm-audio-metadata.md). | varchar(255) |
| **`videoaudioartist`** | Dimensión de servicios de medios de streaming [Artista](/help/components/dimensions/sm-audio-metadata.md). | varchar(255) |
| **`videoaudioauthor`** | Dimensión de servicios de medios de streaming [Autor](/help/components/dimensions/sm-audio-metadata.md). | varchar(255) |
| **`videoaudiolabel`** | Dimensión de servicios de medios de streaming [Etiqueta](/help/components/dimensions/sm-audio-metadata.md). | varchar(255) |
| **`videoaudiopublisher`** | Dimensión de servicios de medios de streaming [Editor](/help/components/dimensions/sm-audio-metadata.md). | varchar(255) |
| **`videoaudiostation`** | Dimensión de servicios de medios de streaming [Estación](/help/components/dimensions/sm-audio-metadata.md). | varchar(255) |
| **`videocampaign`** | Dimensión de servicios de medios de streaming [ID de campaña](/help/components/dimensions/sm-ads.md). | varchar(255) |
| **`videochannel`** | Dimensión de servicios de medios de streaming [Canal de contenido](/help/components/dimensions/sm-core.md). | varchar(255) |
| **`videochapter`** | Dimensión de servicios de medios de streaming [Capítulo](/help/components/dimensions/sm-chapters.md). | varchar(255) |
| **`videocontenttype`** | Dimensión de servicios de medios de streaming [Tipo de contenido](/help/components/dimensions/sm-core.md). | varchar(255) |
| **`videodaypart`** | Dimensión de servicios de medios de streaming [Momento del día](/help/components/dimensions/sm-video-metadata.md). | varchar(255) |
| **`videoepisode`** | Dimensión de servicios de medios de streaming [Capítulo](/help/components/dimensions/sm-video-metadata.md). | varchar(255) |
| **`videofeedtype`** | Dimensión de servicios de medios de streaming [Tipo de fuente de medios](/help/components/dimensions/sm-video-metadata.md). | varchar(255) |
| **`videogenre`** | Dimensión de servicios de medios de streaming [Género](/help/components/dimensions/sm-video-metadata.md). Esta dimensión permite varios valores en la misma visita, delimitados por una coma. | texto |
| **`videolength`** | Dimensión de servicios de medios de streaming [Longitud del contenido (variable)](/help/components/dimensions/sm-core.md). | entero |
| **`videomvpd`** | Dimensión de servicios de medios de streaming [MVPD](/help/components/dimensions/sm-video-metadata.md). | varchar(255) |
| **`videoname`** | Dimensión de servicios de medios de streaming [Nombre del contenido (variable)](/help/components/dimensions/sm-core.md). | varchar(255) |
| **`videonetwork`** | Dimensión de servicios de medios de streaming [Red](/help/components/dimensions/sm-video-metadata.md). | varchar(255) |
| **`videopath`** | Dimensión de servicios de medios de streaming [Ruta de medios](/help/components/dimensions/sm-core.md). | varchar(100) |
| **`videoplayername`** | Dimensión de servicios de medios de streaming [Nombre del reproductor de contenido](/help/components/dimensions/sm-core.md). | varchar(255) |
| **`videotime`** | Métrica de servicios de medios de streaming [Tiempo invertido en contenido](/help/components/metrics/sm-core.md). | entero |
| **`videoqoebitrateaverageevar`** | Dimensión de servicios de medios de streaming [Bitrate promedio](/help/components/dimensions/sm-quality.md). | varchar(255) |
| **`videoqoebitratechangecountevar`** | Dimensión de servicios de medios de streaming [Cambios en bitrate](/help/components/dimensions/sm-quality.md). | varchar(255) |
| **`videoqoebuffercountevar`** | Dimensión de servicios de medios de streaming [Eventos de búfer](/help/components/dimensions/sm-quality.md). | varchar(255) |
| **`videoqoebuffertimeevar`** | Dimensión de servicios de medios de streaming [Duración total del búfer](/help/components/dimensions/sm-quality.md). | varchar(255) |
| **`videoqoedroppedframecountevar`** | Dimensión de servicios de medios de streaming [Fotogramas perdidos](/help/components/dimensions/sm-quality.md). | varchar(255) |
| **`videoqoeerrorcountevar`** | Dimensión de servicios de medios de streaming [Errores](/help/components/dimensions/sm-quality.md). | varchar(255) |
| **`videoqoeextneralerrors`** | Dimensión de servicios de medios de streaming [ID de error externo](/help/components/dimensions/sm-quality.md). Esta dimensión permite varios valores en la misma visita. | texto |
| **`videoqoeplayersdkerrors`** | Dimensión de servicios de medios de streaming [ID de error del reproductor SDK](/help/components/dimensions/sm-quality.md). Esta dimensión permite varios valores en la misma visita. | texto |
| **`videoqoetimetostartevar`** | Dimensión de servicios de medios de streaming [Tiempo para el inicio](/help/components/dimensions/sm-quality.md). | varchar(255) |
| **`videoseason`** | Dimensión de servicios de medios de streaming [Temporada](/help/components/dimensions/sm-video-metadata.md). | varchar(255) |
| **`videosegment`** | Dimensión de servicios de medios de streaming [Segmento de contenido](/help/components/dimensions/sm-core.md). | varchar(255) |
| **`videoshow`** | Dimensión de servicios de medios de streaming [Programa](/help/components/dimensions/sm-video-metadata.md). | varchar(255) |
| **`videoshowtype`** | Dimensión de servicios de medios de streaming [Tipo de programa](/help/components/dimensions/sm-video-metadata.md). | varchar(255) |
| **`videostreamtype`** | Dimensión de servicios de medios de streaming [Tipo de emisión](/help/components/dimensions/sm-core.md). | varchar(255) |
...
| **`visit_keywords`** | Dimensión [Palabra clave de búsqueda](/help/components/dimensions/search-keyword.md). Esta columna utiliza un límite de caracteres no estándar de varchar(244) para dar cabida a la lógica back-end que utiliza Adobe. | varchar(244) |
...
| **`visit_start_page_url`** | Primera URL de la visita. | varchar(255) |
