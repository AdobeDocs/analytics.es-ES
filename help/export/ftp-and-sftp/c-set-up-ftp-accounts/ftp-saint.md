...
La cantidad de tiempo necesaria para que el sistema importe estos archivos varía según una serie de factores. Si un archivo cargado está en el servidor FTP pasados más de tres días, colabore con los usuarios de soporte técnico de su compañía para ponerse en contacto con el Servicio de atención al cliente de Adobe.
...
Al cargar un archivo de Source de datos o clasificación (`.tab` o `.txt`), la carga también requiere que cargue un archivo vacío que tenga el mismo nombre que el archivo de datos que se va a importar, pero con un .`.fin` extensión. El archivo `.fin` es un archivo de finalización. La función de este archivo es informar al sistema de que el archivo de datos se ha cargado completamente en la cuenta de FTP. El archivo `.fin` permite a Adobe saber que la importación ha finalizado.
