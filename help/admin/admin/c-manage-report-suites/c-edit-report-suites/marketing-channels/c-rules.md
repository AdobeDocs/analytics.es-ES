...
Las reglas de procesamiento de canales de marketing averiguan si el visitante que entra cumple los criterios asignados a un canal al procesar cada entrada del visitante en su sitio. Las reglas se procesan en el orden especificado y, cuando se cumple una regla, el sistema detiene el procesamiento de las demás.
...
* Los datos recopilados con estas reglas son permanentes. Las reglas modificadas tras la recopilación de datos no son retroactivas. El Adobe recomienda encarecidamente que revise y considere todas las circunstancias antes de guardar [!UICONTROL Reglas de procesamiento de canal de marketing] para mitigar los datos que se recopilan en canales incorrectos.
...
* Revise las prácticas recomendadas para usar [!UICONTROL Canales de marketing] con [!UICONTROL Atribución].
...
2. Selección de un grupo de informes.
   ...
   Consulte [Ejecutar la configuración automática](/help/components/c-marketing-channels/c-getting-started-mchannel.md).
...
4. Si desea agregar una regla, seleccione en la **[!UICONTROL Agregar nuevo conjunto de reglas]** menú. Si selecciona un canal, recibirá una plantilla de regla y, si selecciona Personalizado, comenzará desde una pizarra en blanco. Ambas opciones permiten modificar el conjunto de reglas según sea necesario.
...
>[!NOTE]
>
>Cualquier campo de texto que defina, como parámetro de cadena de consulta o lista de valores con los que coincida, se evalúan como **sin distinción de mayúsculas y minúsculas** valores. Por ejemplo, si tiene una regla en la que el parámetro de cadena de consulta `cmp = abc123`, todas las variaciones en mayúsculas y minúsculas de ambos `cmp` y `abc123` coincide.

| Término | Definición |
|--- |--- |
| Todos | Activa este canal únicamente cuando se cumplen todos los criterios de la regla. |
| Cualquiera | Activa este canal cuando cualquiera de los criterios de la regla es verdadera. Esta opción solo está disponible si existe más de un criterio en la regla. |
| ID de AMO | El código de seguimiento principal utilizado por las integraciones de Advertising Cloud y Advertising Analytics. Cuando se habilita una de estas integraciones, puede usar el prefijo del código de seguimiento para identificar canales específicos de Advertising Cloud. Utilice “AMO ID” para comenzar con “AL” para la búsqueda, “AC” para la visualización o “AO” para Social. Cuando se utiliza el ID de AMO en los canales de marketing, las métricas de clics, costes e impresiones se pueden atribuir al canal correcto (cuando no se configuran, estas métricas se dirigen a Directas o a Ninguna). |
| AMO ED ID | Código de seguimiento secundario utilizado por Advertising Cloud. El propósito principal de este código de seguimiento es servir como clave para enviar datos de vuelta a Ad Cloud. Sin embargo, también se puede utilizar para identificar Mostrar clics respecto a Mostrar vistas si desea verlas como dos canales de marketing independientes. Esto se puede hacer configurando que la lógica del canal de marketing para &quot;AMO EF ID&quot; termine en `:d` para las visualizaciones de pulsaciones o &quot;AMO EF ID&quot; termina con `:i` para Mostrar vistas. Si no desea dividir Mostrar en dos canales, utilice la dimensión de ID de AMO en su lugar. |
...
| Dominio de referencia | El [Dominio de referencia](/help/components/dimensions/referring-domain.md) dimensión |
| Parámetro de cadena de consulta | Utilice un parámetro de cadena de consulta individual. Solo se puede especificar un parámetro de cadena de consulta por criterio. Para agregar más parámetros de cadena de consulta, utilice `ANY` como operador, agregue parámetros de cadena de consulta a la regla. |
...
A continuación se muestra el orden recomendado para las reglas de canal y las definiciones de ejemplo:
...
La búsqueda natural se produce cuando los visitantes encuentran el sitio web a través de un motor de búsqueda y este clasifica el sitio sin que usted haya pagado para ello.

El Adobe determina el tráfico de búsqueda en función de una búsqueda interna de motores de búsqueda. Si un remitente del reenvío coincide con los criterios de un motor de búsqueda, determina si es de pago o natural mediante [Detección de búsqueda de pago](../general/paid-search-detection/paid-search-detection.md) reglas que ha configurado. Una visita se considera búsqueda natural cuando no coincide con ninguna regla de detección de búsqueda de pago.
