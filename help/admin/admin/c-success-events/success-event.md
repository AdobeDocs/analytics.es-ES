...
5. Busque el evento deseado y modifique la lista desplegable Registro [!UICONTROL de eventos] únicos para que [!UICONTROL Registrar una vez por visita] o [!UICONTROL usar ID]de evento.
...
| Registro de un solo evento | **Registrar una vez por visita**: Vincula el evento determinado con la sesión del visitante. Se omiten los recuentos posteriores de un evento determinado en la misma visita. Este tipo de serialización de eventos no requiere ningún cambio de implementación.<br>**Usar ID **de evento: Vincula el evento determinado con una ID personalizada. Se omiten los recuentos posteriores de un evento determinado con la misma ID de evento. Este tipo de serialización de eventos requiere un ID personalizado en las visitas para anular la duplicación de valores. Consulte la serialización[de ID de](../../../implement/vars/page-vars/events/event-serialization.md)evento en la guía de usuario Implementación. |
