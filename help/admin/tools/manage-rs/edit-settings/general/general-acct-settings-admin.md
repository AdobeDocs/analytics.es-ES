...
| Dirección URL base | Especifica el sitio web principal del grupo de informes. La URL básica no afecta al filtrado de referentes. En su lugar, use [filtros de URL internos](/help/admin/tools/manage-rs/edit-settings/general/internal-url-filter-admin.md). |
| Zona horaria | Determina la fecha y la hora asociadas con los datos del informe. Si la zona horaria seleccionada cumple el horario de verano, los datos horarios se ajustan automáticamente para reflejarlo. Si se cambia la zona horaria de un grupo de informes activo, se creará un pico o un hueco en los datos del informe. Adobe recomienda realizar este cambio durante las horas de menor actividad para minimizar el impacto. |
