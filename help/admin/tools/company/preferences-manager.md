---
description: La página Administrador de preferencias permite configurar cómo se procesan los resultados de los informes de Excel y de valores separados por comas (CSV).
title: Administrador de preferencias
feature: Admin Tools
uuid: 46488e24-57a5-4f99-997c-0cfb61625008
exl-id: 9ead47e3-ab43-4789-ba8e-a6b7be7f750e
TQID: https://experienceleague.adobe.com/qcMmwuMQobNAFwEY-8rJhr7HY11N2-ouIoigHKG2c7w
product_v2: id: e55547f1-a1ff-40c6-8978-026e40ab7fa4
feature_v2: id: ff9b434a-2221-4df7-81d1-5bcbf5f80bce
role_v2: id: b69b2659-1057-424e-8fc5-ed9e016dc554id: c66ffd68-0f65-42bb-aa23-b4020f12e0bd
topic_v2: id: eddd9b14-83bd-4ff4-9072-54a4a484abb7
source-git-commit: ff16e07c7a2b75e9c6cc09e8255a7ea7e4c6f0c8
workflow-type: tm+mt
source-wordcount: 66
ht-degree: 16%

---

# Administrador de preferencias

La página Administrador de preferencias permite configurar cómo se procesan los resultados de los informes de Excel y de valores separados por comas (CSV).

**[!UICONTROL Analytics]** > **[!UICONTROL Administración]** > **[!UICONTROL Toda la administración]** > **[!UICONTROL Configuración de la compañía]** > **[!UICONTROL Preferencias]**

* Utilice un formato de fecha obsoleto para el CSV (30 de agosto de 20XX)
* Utilice el formato de fecha ISO 8601 para el CSV (20XX-08-30)
