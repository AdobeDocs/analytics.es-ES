...
Todos los clientes deben conectarse mediante SFTP con una contraseña como alternativa.
...
#### Agregue la clave pública SSH de Adobe al archivo [!DNL `authorized_keys`] y cárguela en su servidor FTP
...
Debe agregar esta clave pública al mismo archivo [!DNL `authorized_keys`] al que agregó anteriormente la clave de descarga de su organización (la que generó en [Paso 1: Generar las claves SSH de su organización para descargar datos](#step-1-generate-your-organizations-ssh-keys-for-downloading-data)).

Para agregar la clave pública SSH de Adobe al archivo [!DNL `authorized_keys`] y cargarla en el servidor FTP:

1. Inicie sesión en la estación de trabajo donde descargue datos del servidor FTP.

1. Abra el archivo [!DNL `authorized_keys`] y agréguele la clave de carga de Adobe. Este archivo ya debería contener la clave de descarga de su organización en [Paso 1: Genere las claves SSH de su organización para descargar datos](#step-1-generate-your-organizations-ssh-keys-for-downloading-data).
...
1. En la ficha [!UICONTROL **Ubicaciones**], seleccione [!UICONTROL **Agregar ubicación**].
...
Para obtener instrucciones detalladas, consulte [Configurar ubicaciones de importación y exportación en la nube](https://experienceleague.adobe.com/en/docs/analytics/components/locations/configure-import-locations).
