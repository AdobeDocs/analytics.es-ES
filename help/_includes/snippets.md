...
|---------|----------|---------|---------|
| **Secuencia de páginas predefinido** | Sí</br>Combina análisis exploratorios y predefinidos. La ruta final se utiliza cuando se utilizan nodos predefinidos en la ruta (los visitantes se cuentan siempre y cuando pasen de un nodo predefinido al otro). También se pueden mostrar los siguientes nodos inmediatos (no posibles). | </br>Sí La ruta puede ser una ruta eventual o puede limitarse al siguiente punto de contacto | No |
...
| **Muestra dónde abandonaron las personas (abandonaron) y continuaron (abandonaron)** | Sí</br>Muestra recorridos predefinidos y exploratorios | Sí</br>Muestra recorridos predefinidos | </br>Sí Espectáculos para viajes exploratorios |
| **Viajes lineales** | Sí | Sí | No |
| **Viajes no lineales con múltiples puntos de entrada y rutas** | Sí | No | Sí |
| **Métrica primaria** | Cualquier Métrica, incluidas las métricas calculadas | Solo sesión o persona | Solo ocurrencias (vistas de ruta) |
